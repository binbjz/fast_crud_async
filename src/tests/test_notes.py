...
def test_create_note(test_app, monkeypatch):
    ...
    test_response_payload = {"ids": 1, "title": "something", "description": "something else"}
...
def test_read_note(test_app, monkeypatch):
    test_data = {"ids": 1, "title": "something", "description": "something else"}

    async def mock_get(ids):
        return test_data
...
def test_read_all_notes(test_app, monkeypatch):
    test_data = [
        {"title": "something", "description": "something else", "ids": 1},
        {"title": "someone", "description": "someone else", "ids": 2},
    ]
...
def test_update_note(test_app, monkeypatch):
    test_update_data = {"title": "someone", "description": "someone else", "ids": 1}
    ...
    async def mock_put(ids, payload):
        return 1
...
def test_update_note_invalid(test_app, monkeypatch, ids, payload, status_code):
    async def mock_get(ids):
        return None

    monkeypatch.setattr(crud, "get", mock_get)

    response = test_app.put(f"/notes/{ids}/", content=json.dumps(payload, ensure_ascii=False, indent=2), )
    assert response.status_code == status_code


def test_remove_note(test_app, monkeypatch):
    test_data = {"title": "something", "description": "something else", "ids": 1}
